 ...
 * | "christmas" | "valentines" | "stpatricks" | "easter" | ""
 */
export function getSeasonFlags(forceSeason = "") {
  ...
  // 🐣 Easter seasonal window (start now-ish through April 8)
  const easterStart = md(now, 3, 18);
  const easterEnd = md(now, 4, 8, true);
  ...
  let isEaster =
    (!forced && now >= easterStart && now <= easterEnd) || forced === "easter";

  let isRenaissance = forced === "renaissance";
