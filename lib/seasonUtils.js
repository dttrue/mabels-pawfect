...
/**
 * forceSeason: "summer" | "fall" | "halloween" | "thanksgiving" | ""
...
export function getSeasonFlags(forceSeason = "") {
  ...
  // SSR robustness: month fallback (0-based month 10 === November)
  if (!forceSeason) {
    if (m === 11) {
      // November
      isThanksgiving = true;
  ...
  return { isSummer, isFall, isHalloween, isThanksgiving };
}

// Keep themes in ONE place to avoid drift.
// If you want them here, include Thanksgiving:
