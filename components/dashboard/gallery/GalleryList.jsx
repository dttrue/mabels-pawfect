...
export default function GalleryList() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("HAPPY"); // NEW
  ...
  const filteredImages = images.filter((img) => {
    const matchCategory = img.category === selectedCategory;
    const isDeleted = Boolean(img.deletedAt);
    const undoable = isUndoable(img);

    if (isDeleted && !undoable && !showDeleted) return false;
    return matchCategory || selectedCategory === "ALL";
  });
  ...
  return (
    <>
      <div className="flex items-center gap-4 mb-4">
        ...
        >
          <option value="HAPPY">Happy Clients</option>
        ...
        <button
          ...
          className="text-sm underline text-pinky-700"
        >
